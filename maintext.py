...
class MainText(object):
    """ Class to extract the main text (excluding template, tables, etc.) from wikimarkup """
    ...
    def line_debug(self, logger, lineno, indent, tag, clip):
        logger.debug('%5d:%-10s%s', lineno, ''.join([' ' for s in range(indent - 1)]) + tag, clip.replace('\n', '\\n'))
                ...
                if (buf[1] == '\n' or buf[1] == '0') and buf[0] == '{' and c == '|':
                    ...
                    if buf[0] == '}':
                        ...
                        if ptree[-1] == 'template':
                            ...
                            self.line_debug(logger, i, len(ptree), 'Extra set of }}s!', souped[i-10:i+1])
                            logger.warn('Found extra set of }}s! Parse tree is: %s', ','.join(ptree))
